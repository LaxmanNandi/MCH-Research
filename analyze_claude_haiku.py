"""
MCH v8.1 Comprehensive Analysis - Claude Haiku 4.5 (100 Trials)
...
def analyze_claude_haiku():
    print("=" * 70)
    print("MCH v8.1 COMPREHENSIVE ANALYSIS - CLAUDE HAIKU 4.5")
    print("=" * 70)

    # Load results
    results = load_results("mch_results_claude_haiku_100trials.json")
    ...
    print("\n" + "=" * 70)
    print("PRIMARY HYPOTHESIS TEST: DRCI vs Cold")
    print("=" * 70)
    ...
    print(f"\nMean DRCI (True - Cold): {mean_drci:.4f}")
    ...
    if p_val < 0.05:
        if mean_drci > 0:
            classification = "CONVERGENT (Positive DRCI, p < 0.05)"
            pattern = "Convergent"
        else:
            classification = "SOVEREIGN (Negative DRCI, p < 0.05)"
            pattern = "Sovereign"
    else:
    ...
    # Scrambled condition
    print("\n" + "-" * 50)
    print("SECONDARY: DRCI vs Scrambled")
    print("-" * 50)
    ...
    print(f"Mean DRCI (True - Scrambled): {mean_drci_s:.4f}")
    ...
    print(f"\nFirst 50 Trials:")
    print(f"  Mean DRCI: {mean_f50:.4f} (SD: {std_f50:.4f})")

    print(f"\nLast 50 Trials:")
    print(f"  Mean DRCI: {mean_l50:.4f} (SD: {std_l50:.4f})")
    ...
    fig.suptitle(f'MCH v8.1 Analysis: Claude Haiku 4.5 (n={n})\nPattern: {pattern} | Cohen\'s d = {cohens_d:.3f} | p = {p_val:.4f}',
                 fontsize=14, fontweight='bold')

    trial_nums = list(range(1, n + 1))

    # Panel 1: DRCI over trials (cold)
    ax1 = axes[0, 0]
    ...
    ax1.set_title('DRCI Over Trials (vs Cold)')
    ax1.legend(fontsize=8)
    ax1.grid(True, alpha=0.3)

    # Panel 2: DRCI histogram
    ax2 = axes[0, 1]
    ...
    ax2.set_title('DRCI Distribution')
    ...
    bp = ax5.boxplot([delta_rci_cold, delta_rci_scrambled],
                      tick_labels=['vs Cold', 'vs Scrambled'],
                      patch_artist=True)
    bp['boxes'][0].set_facecolor('steelblue')
    bp['boxes'][1].set_facecolor('coral')
    ax5.axhline(y=0, color='black', linestyle='--', linewidth=1)
    ax5.set_ylabel('DRCI')
    ax5.set_title('DRCI by Control Condition')
    ...
    # Panel 8: Rolling mean DRCI (window=10)
    ax8 = axes[2, 1]
    window = 10
    ...
    ax8.set_ylabel('Rolling Mean DRCI')
    ax8.set_title(f'Rolling Mean DRCI (window={window})')
    ...
    Key Finding:
      Claude Haiku shows NEUTRAL
      context utilization - no
      significant difference between
      True and Cold conditions.
    """
    ...
    # Save figure
    output_file = 'claude_haiku_mch_analysis.png'
    ...
    print(f"""
    CLAUDE HAIKU 4.5 - MCH v8.1 EXPERIMENT RESULTS
    ================================================

    Pattern Classification: {classification}

    Key Finding: Claude Haiku exhibits a NEUTRAL pattern,
    meaning there is no statistically significant difference
    between True (with context) and Cold (no context) responses.

    Statistical Evidence:
    - DRCI = {mean_drci:.4f}
    - p = {p_val:.6f} (NOT significant, p >= 0.05)
    - Cohen's d = {cohens_d:.3f} ({effect_interp} effect)
    - 95% CI INCLUDES zero: [{ci_95[0]:.4f}, {ci_95[1]:.4f}]

    Interpretation:
    Unlike Gemini Flash (Sovereign, -DRCI) or Claude Opus 4
    (Convergent, +DRCI), Claude Haiku shows no significant
    context-dependent behavior. Its responses are similar
    whether given conversation history or not.

    This suggests Claude Haiku may process context differently
    or rely less on conversational history for its responses.
    """)
