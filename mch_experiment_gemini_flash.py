...
class ExperimentConfig:
    participant_id: str = "gemini_flash_100trials"
    n_trials: int = 100
    ...
    def __post_init__(self):
        if self.models_to_test is None:
            self.models_to_test = ["gemini-2.5-flash"]
...
class MCHExperimentGemini:
    ...
    def run_experiment(self, prompts: List[str]) -> Dict:
        print("\n" + "="*60)
        print("MCH v8.1 EXPERIMENT - Gemini 2.5 Flash")
        print("="*60)
...
def main():
    ...
    config = ExperimentConfig(
        participant_id="gemini_flash_100trials",
        n_trials=100,
    ...
    experiment = MCHExperimentGemini(config)
    results = experiment.run_experiment(sequential_prompts)
    experiment.save_results("mch_results_gemini_flash_100trials.json")

    print("\n" + "="*60)
    print("GEMINI 2.5 FLASH EXPERIMENT COMPLETE")
    print("="*60)
