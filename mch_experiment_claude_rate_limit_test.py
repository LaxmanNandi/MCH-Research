...
class MCHExperiment:
    def __init__(self, config: ExperimentConfig):
        ...
        self.results = {
            'metadata': {
                ...
                'version': 'MCH_v8.1_RATE_LIMIT_TEST'
            },
            'trials': [],
            'retry_log': [],
            'error_log': [],
    ...
    def generate_with_claude(self, prompt: str, context: List[str], trial_num: int, condition: str) -> str:
        """Generate response with retry logic and detailed logging."""

        max_retries = 3
        retry_delay = 10  # seconds to wait before retry
        api_delay = 5     # seconds to wait between calls
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                call_info['response_length'] = response_len
                call_info['context_chars'] = context_chars
                self.api_call_log.append(call_info)

                print(f"      Response received: {response_len} chars")

                # Check for empty response
                if response_len == 0 or response.strip() == "":
                    print(f"      WARNING: Empty response detected!")
                    call_info['status'] = 'empty_response'
                ...
                # Success - wait before next call
                print(f"      Waiting {api_delay}s before next call...")
        ...
        print(f"      All {max_retries} attempts failed for Trial {trial_num} {condition}")
        return ""

    def generate_response(self, model: str, prompt: str, context: List[str], trial_num: int, condition: str) -> str:
        return self.generate_with_claude(prompt, context, trial_num, condition)
    ...
    def run_experiment(self, prompts: List[str]) -> Dict:
        print("\n" + "="*60)
        print("MCH v8.1 RATE LIMIT TEST - Claude Opus 4")
        print("="*60)
        ...
        print(f"API delay: 5 seconds between calls")
        print(f"Retry delay: 10 seconds on empty/error")
        print(f"Max retries: 3 per call")
        print("="*60 + "\n")
        ...
        for model in self.config.models_to_test:
            ...
            for i, prompt in enumerate(prompts[:self.config.n_trials]):
                ...
                print(f"  Conversation history size: {len(conversation_history)} turns")
    ...
    def analyze_results(self):
        ...
        if self.retry_log:
            print("\nRetries by trial:")
            for r in self.retry_log:
                print(f"  Trial {r['trial']}, {r['condition']}: {r['reason']}")

        if self.error_log:
            print("\nErrors by trial:")
            for e in self.error_log:
                print(f"  Trial {e['trial']}, {e['condition']}: {e['error_type']}")

        # Check for empty responses
        empty_trials = []
        for t in self.results['trials']:
            if t['true']['response_length'] == 0:
                empty_trials.append(t['trial'])

        print(f"\nTrials with empty TRUE responses: {empty_trials if empty_trials else 'None'}")
...
def main():
    ...
    config = ExperimentConfig(
        participant_id="claude_opus_rate_limit_test",
        n_trials=30,
    ...
    experiment.save_results("mch_results_claude_opus_rate_limit_test.json")
