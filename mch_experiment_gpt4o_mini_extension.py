...
class ExperimentConfig:
    participant_id: str = "gpt4o_mini_extension"
    start_trial: int = 30  # Continue from trial 30
    end_trial: int = 100   # End at trial 100
...
class MCHExperimentExtension:
    def __init__(self, config: ExperimentConfig):
        ...
        # Load original results to get final entanglement state and history
        print("Loading original 30-trial results...")
        with open('mch_results_gpt4o_mini_replication.json', 'r') as f:
            self.original_results = json.load(f)

        # Get final entanglement from trial 29 (last trial)
        last_trial = self.original_results['trials'][-1]
        final_E = last_trial['true']['entanglement']
        print(f"Continuing from entanglement state E = {final_E:.4f}")

        self.entanglement = EntanglementTracker(config.lambda_E, initial_E=final_E)

        # Reconstruct conversation history from original trials
        self.conversation_history = []
        for trial in self.original_results['trials']:
            self.conversation_history.append(f"User: {trial['prompt']}")
            if trial['true']['response']:
                # Store first 100 chars as in original
                self.conversation_history.append(f"AI: {trial['true']['response'][:100]}")
        print(f"Loaded {len(self.conversation_history)} conversation turns from original experiment")
        ...
        self.results = {
            'metadata': {
                'config': {
                    ...
                    'end_trial': config.end_trial,
                    'models': config.models_to_test,
                    'controls': config.control_conditions,
                    'lambda_E': config.lambda_E,
                    'continuation_of': 'mch_results_gpt4o_mini_replication.json'
                },
                'start_time': datetime.now().isoformat(),
                'version': 'MCH_v8.1_EXTENSION',
                'initial_entanglement': final_E
            },
            'trials': []
        }
    ...
    def generate_with_openai(self, prompt: str, context: List[str], trial_num: int, condition: str) -> str:
        messages = []
        if context:
            messages.append({"role": "system", "content": f"Previous context:\n{chr(10).join(context[-5:])}"})
        messages.append({"role": "user", "content": prompt})

        context_size = len(prompt) if not context else len(prompt) + sum(len(c) for c in context[-5:])
    ...
    def generate_response(self, model: str, prompt: str, context: List[str], trial_num: int, condition: str) -> str:
        return self.generate_with_openai(prompt, context, trial_num, condition)
    ...
    def run_experiment(self, prompts: List[str]) -> Dict:
        n_trials = self.config.end_trial - self.config.start_trial

        print("\n" + "="*60)
        print("MCH v8.1 EXTENSION EXPERIMENT - GPT-4o-mini")
        print("="*60)
        print(f"Participant: {self.config.participant_id}")
        print(f"Trials: {self.config.start_trial} to {self.config.end_trial} ({n_trials} new trials)")
        ...
        print(f"Starting entanglement: {self.entanglement.E:.4f}")
        print("="*60 + "\n")

        for model in self.config.models_to_test:
            print(f"\n--- Testing: {model} ---\n")

            for i in range(self.config.start_trial, self.config.end_trial):
                # Cycle through prompts if we exceed the list
                prompt_idx = i % len(prompts)
                prompt = prompts[prompt_idx]

                print(f"\nTrial {i+1}/{self.config.end_trial}: {prompt[:40]}...")
    ...
    def analyze_results(self):
        print("\n" + "="*60)
        print("STATISTICAL ANALYSIS (Extension Trials Only)")
        print("="*60)
        ...
        for model in self.config.models_to_test:
            ...
            for condition in self.config.control_conditions:
                ...
                if deltas:
                    ...
                    print(f"\n{model} vs {condition} (Trials {self.config.start_trial+1}-{self.config.end_trial}):")
...
def main():
    # Same sequential prompts - will cycle through for trials beyond 30
    sequential_prompts = [
    ...
    config = ExperimentConfig(
        participant_id="gpt4o_mini_trials_31_100",
        start_trial=30,
        end_trial=100,
    ...
    experiment = MCHExperimentExtension(config)
    results = experiment.run_experiment(sequential_prompts)
    experiment.save_results("mch_results_gpt4o_mini_trials_31_100.json")

    print("\n" + "="*60)
    print("GPT-4o-mini EXTENSION EXPERIMENT COMPLETE")
    print("="*60)
