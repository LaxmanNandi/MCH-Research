...
The variance of responses at position $p$ under condition $c$ is computed as the
mean variance across all embedding dimensions, taken across $N = 50$ trials:

\begin{equation}
    \text{Var}_{p}^{(c)} = \frac{1}{D} \sum_{k=1}^{D}
    \text{Var}_{i}\!\left( e_{i,p,k}^{(c)} \right)
\end{equation}

where $D$ is the embedding dimensionality (384 for MiniLM), $e_{i,p,k}^{(c)}$ is
the $k$-th dimension of the embedding for trial $i$ at position $p$ under condition
$c$, and $\text{Var}_{i}$ denotes variance across the 50 trials. In code:
\texttt{np.mean(np.var(embs\_at\_p, axis=0))}.
...
This formulation captures how spread out the response embeddings are across trials
at each position, averaged over all embedding dimensions. It is identical in both
\texttt{paper6\_conservation\_law.py} and \texttt{compile\_paper6\_data.py},
ensuring consistency across all 24 model-domain runs.
