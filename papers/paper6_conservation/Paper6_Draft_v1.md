...
| DIVERGENT | High (2.6-7.5) | Low-Medium | Llama Scout | Divergent and inconsistent |
...
- **DIVERGENT** models (Llama Scout, Llama Maverick) allocate capacity toward output variance at the expense of context sensitivity, with Llama Scout exhibiting the most extreme trade-off (ΔRCI = 0.323, Var_Ratio = 1.610). This allocation pattern correlates with incomplete task coverage at the critical summarization position.
- **RICH** (Qwen3 235B) shows moderate excess variance (1.334) with moderate ΔRCI (0.328), sitting between IDEAL and DIVERGENT on the hyperbola.

![Figure 4: Taxonomy overlay on conservation constraint](figures/fig4_taxonomy_overlay.png)

*Figure 4.* Four-class predictability taxonomy overlaid on the conservation constraint. All classes follow the domain-specific hyperbola, representing different allocation strategies within the same domain-specific capacity. IDEAL models occupy the balanced center; DIVERGENT models allocate capacity toward output variance; EMPTY models exhibit an elevated total product.
...
5. Laxman, M. M. (2026e). Stochastic incompleteness in LLM summarization: A predictability taxonomy for clinical AI deployment. In preparation.
