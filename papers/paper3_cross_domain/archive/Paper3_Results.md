...
We analyzed position-dependent Delta Relational Coherence Index (ΔRCI) across **12 state-of-the-art LLMs** in a cross-domain experimental design: 4 models (closed-source APIs) in philosophy and 8 models (7 open-source + 1 closed) in medical reasoning. This yields 12 model-domain runs covering 30 conversational positions (360 position measurements). Each position was evaluated under three conditions (TRUE, COLD, SCRAMBLED) with 50 independent trials per model, generating **~54,000 responses with complete text preserved**. ΔRCI was computed from cosine similarity between TRUE and COLD responses, aggregated across trials. Disruption Sensitivity (DS) quantifies the value of context presence versus ordering:
...
| **Closed-Goal** | Type 2 | Guideline-bounded | Medical (STEMI diagnosis) | U-shaped (diagnostic trough at P10-25) | 0.31-0.37 | **Extreme spike** (Z > +2.7, task enablement) | 8/8 models (100%) |
...
2. **Safety-Critical Insight**: Type 2 tasks show **task enablement** (not just performance enhancement) at summarization positions:
   - Medical P30: Mean Z = +3.47 (all 8 models show extreme outlier)
...
4. **Architectural Robustness**:
   ...
   - Type 2: 8/8 medical models show U-shaped pattern + P30 spike (100% consistency)
   - Pattern is architecture-invariant (holds across GPT, Claude, Gemini, DeepSeek, Llama, Mistral, Qwen, Kimi)
   - **Distinction is temporal + functional, NOT magnitude-based** (ΔRCI ranges overlap: Philosophy 0.27-0.33, Medical 0.31-0.37)
...
**Medical (STEMI reasoning):** Across 8 models (DeepSeek V3.1, Gemini Flash, Kimi K2, Llama 4 Maverick/Scout, Mistral Small 24B, Ministral 14B, Qwen3 235B), ΔRCI followed a U-shaped pattern: Early = 0.347, Mid = 0.311 (diagnostic trough), Late = 0.371. Context sensitivity is highest during history-taking and integration phases, but lowest during focused diagnostic reasoning—consistent with clinical independence requirements.
...
Both closed-source APIs (GPT-4o, Claude Haiku, Gemini Flash) and open-source models (DeepSeek, Kimi, Llama, Mistral, Qwen) exhibit domain-consistent dynamics. The medical diagnostic trough (positions 11-20) appears across all 8 architectures, suggesting a shared computational strategy for clinical reasoning.
...
- **P30 behavior**: **Extreme spike** (task enablement, Z > +2.7)
- **Examples**: Medical diagnosis with clinical guidelines, legal contract analysis, guideline-based code review
- **ΔRCI range**: 0.31-0.37 (Early: 0.347, Mid: 0.311 (trough), Late: 0.371)
- **Evidence**: 8/8 medical models show identical pattern (100% consistency)
...
### Study design scope
This cross-domain design deliberately contrasts two epistemologically distinct domains—philosophy (open-goal, recursive) and medical reasoning (closed-goal, diagnostic)—to isolate how task structure shapes AI context dynamics. We selected 12 representative models spanning closed-source APIs and open-source architectures, with **complete response text preservation** enabling qualitative validation of quantitative patterns.

Key design features:
- **Cross-domain contrast**: Philosophy (4 models) vs Medical (8 models) allows direct comparison of open-goal vs closed-goal reasoning
- **Response text preservation**: ~54,000 full responses enable qualitative inspection of summarization failures, diagnostic reasoning, and abstraction patterns
