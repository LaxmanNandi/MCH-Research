...
\begin{itemize}
    \item Mean $Z = +3.47 \pm 0.51$ (all $> +2.7$)
...
This 8/8 vs 0/4 split ($p < 0.001$, Fisher's exact test) suggests that closed-goal summarization \textit{requires} accumulated context for task execution, while open-goal philosophical synthesis can proceed without context dependence at the final position.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{fig3_zscores.png}
    \caption{P30 Z-score analysis. Medical domain (left): all 8 models exceed $Z = +2.0$ (orange dashed line). Philosophy domain (right): no model exceeds threshold. 8/8 vs 0/4 split, $p < 0.001$ (Fisher's exact test).}
