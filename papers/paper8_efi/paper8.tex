...
\textbf{Paper 8 of the MCH Research Program} --- This paper extends the MCH framework from output-layer behaviour to input encoding. Using the Encoding Fidelity Index (EFI), we show that Shannon's (1948) implicit assumption of encoding fidelity fails systematically for non-English languages in LLMs, producing a novel failure mode we term \emph{Coherent Misalignment}. The conservation law $K = \Delta\text{RCI} \times \text{Var\_Ratio}$ from Papers 2--7, established across 14 models and 3 domains, is structurally orthogonal to semantic correctness --- $K \perp \text{Truth}$ --- since it measures variance structure, not content; direct verification under non-English encoding is a target for future work.
}}
\end{center}
...
In resource-constrained clinical settings worldwide, large language models are increasingly deployed for decision support --- differential diagnosis, drug interaction checks, guideline lookups. LLMs encode substantial clinical knowledge \citep{singhal2023}, yet this knowledge is benchmarked almost exclusively in English. Jin et~al.\ \citep{jin2024} demonstrate that LLMs give measurably higher-quality healthcare responses in English than in non-English languages across diverse clinical queries. Clinicians in these settings interact with patients who present in local languages: Kannada, Tamil, Hindi, Swahili, Bahasa, or one of hundreds of other languages spoken by the populations these systems are intended to serve. The AI responses are grammatically fluent, contextually appropriate, and confidently delivered. They are also, with disturbing frequency, semantically wrong in ways that would be invisible without domain expertise.
...
For LLMs operating on non-English text, this assumption fails. Joshi et~al.\ \citep{joshi2020} document that most of the world's languages are severely under-resourced in NLP systems; Kannada falls into this category. Tokenisers fragment non-English words into 2--15$\times$ more subword units than English equivalents \citep{petrov2023,liang2025}, imposing a ``token tax'' that compounds through attention layers \citep{lundin2025}. Cross-lingual representation models show systematic degradation for typologically distant languages \citep{pires2019,conneau2020}, and multilinguality itself introduces capacity trade-offs that disproportionately harm low-resource languages \citep{chang2024}. Zouhar et~al.\ \citep{zouhar2023} demonstrate that tokenization efficiency can be quantified as an information-theoretic channel property --- good tokenizers maximise Shannon entropy of the subword distribution. For Indian languages, this channel is structurally degraded before generation begins. Multilingual medical benchmarks show $\sim$10 percentage-point accuracy drops for non-English queries \citep{alonso2024}, a pattern confirmed across 70 languages by Ahuja et~al.\ \citep{ahuja2023}, but no prior work has measured the encoding fidelity loss directly or connected it to output variance structure.
...
\textbf{Scope of evidence:} The $K \perp \text{Truth}$ claim follows from the structural nature of the conservation law established across 14 models and 3 domains in Papers 2--6~\citep{laxman2026b,laxman2026f}. $K$ measures variance structure --- how context sensitivity and output consistency trade off --- which is independent of semantic content. Paper~8 does not directly compute $K$ under non-English conditions, which would require running the full MCH protocol (50 trials $\times$ 30 positions) in Kannada. The claim is a theoretical consequence of $K$'s structural definition; direct verification in non-English conditions is a target for future work.
...
Clinical AI deployment in linguistically diverse and resource-constrained populations requires external truth anchoring, not just internal consistency checks. The encoding fidelity gap is a new failure mode that extends Shannon's original model. For 1.5 billion non-English speakers worldwide, measurement of encoding fidelity must precede deployment.
...
\begin{enumerate}
    ...
    \item \textbf{Direct $K$ verification:} Run the full MCH protocol (50 trials $\times$ 30 positions $\times$ TRUE/COLD conditions) in Kannada to directly verify that $K(\text{Medical})$ is conserved under degraded encoding.
...
\newblock \emph{Preprints.org}, DOI: 10.20944/preprints202601.1881.v2.
...
\newblock \emph{Preprints.org}, DOI: 10.20944/preprints202602.1674.v1.
