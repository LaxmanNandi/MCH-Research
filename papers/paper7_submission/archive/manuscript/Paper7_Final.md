...
| CUD Pilot (new) | 4 LLMs | Medical (P30), Philosophy (P15) | 50 per config | TRUE, COLD, TRUNCATED(K) |
| Paper 6 Legal (preliminary) | 5 LLMs (2 complete, 3 pending) | Legal (P30) | 50 per config | TRUE, SCRAMBLED, COLD |
...
| Qwen3 235B | 1 | Flat at ~97% | Recency-dominant |
| Llama 4 Maverick | >1 | Rising: 82% → 98% | Genuine depth utilization |

#### 3.1.2 Domain Contrast

- Medical: CUD is meaningful (ΔRCI ~0.80-0.94)
- Philosophy: CUD is undefined (ΔRCI ~0.13-0.22, K-curves are noise)
...
|-----------|-------------------|----------------|
| TRUE - COLD | +1.88 to +3.30 | Full context spike (documented) |
| SCRAMBLED - COLD | +0.97 to +3.56 | Content-only spike (equally strong) |
...
Where:
  ...
  Open-goal domains:   Content Fraction ≈ 38%
```
...
Unlike ΔRCI Content Fraction (which differs by domain, p=0.00073), the variance decomposition shows **no domain difference** in either VR_Content (p=0.46) or VR_Order (p=0.87). Domains shape *how much* context changes responses (sensitivity), not *how stable* those changes are (variance).
...
|--------|---------|-----------|---------|
| Medical (N=8) | 0.429 (CV=0.17) | 1.484 (CV=0.25) | 0.117 (CV=0.29) |
| Philosophy (N=6) | 0.301 (CV=0.17) | 1.590 (CV=0.75) | 0.096 (CV=0.49) |

K_total differs significantly between domains:
- K_total: U=46, p=0.003

However, the individual variance components do **not** differ by domain:
- K_content: U=26, p=0.852 (not significant)
- K_order: U=33, p=0.282 (not significant)

#### 3.4.2 Interpretation

The conservation constant K is not atomic — it has internal structure. But the domain separation in K_total is driven entirely by **ΔRCI (the sensitivity term)**, not by the variance components. VR_Content and VR_Order are domain-invariant (Section 3.3.3), so K_content and K_order inherit their domain difference from ΔRCI alone. The variance machinery — how content amplifies and order suppresses — operates identically across domains. Domain specificity resides in *what is said* (sensitivity), not in *how variable it is* (stability).
...
- Mean Arc ≈ 1.80 (N=6 models with response text available)
...
Philosophy shows both high Order Fraction (~62%) and high Exploration Arc (~1.80). This is consistent: models in open-goal domains rely on sequential argument building (order-dependent) and expand their response space as more argument structure is available (exploration).
...
| Domain difference | Yes (p=0.00073) | No (p=0.46, p=0.87) |
...
Paper 6 established ΔRCI × Var_Ratio ≈ K(domain). We show K is not atomic but decomposes into K_content and K_order. Crucially, K_total differs between domains (p=0.003) but the variance components VR_Content and VR_Order are domain-invariant. This means the domain separation in K comes entirely from ΔRCI — the sensitivity term — not from the variance machinery. The variance decomposition (content amplifies, order suppresses) operates as a universal mechanism across all domains. Domain specificity resides in what the model says, not in how variable it is.
...
Preliminary Legal domain data (Llama 4 Maverick, 50 trials) confirms the information hierarchy TRUE > SCRAMBLED > COLD in a third domain (Paper 6, in progress), consistent with Paper 2's universal finding across 25 configurations.

### 4.7 Connection to the Conservation Constraint

Paper 6 established ΔRCI × Var_Ratio ≈ K(domain). The Content-Order decomposition reveals K has internal structure (Section 3.4), but the domain separation in K is driven entirely by the sensitivity term (ΔRCI), not by the variance components. The variance machinery — content amplifies, order suppresses — is universal across domains.

Paper 6 (in progress) will test the conservation constraint and Content-Order decomposition across three new domains — Legal, Technical, and Applied Ethics — using the same three-condition protocol. Preliminary Legal data (2/5 models complete) already confirms the information hierarchy. Full variance decomposition and K(Legal) computation await complete data collection across all five models and domains.

### 4.8 Limitations

1. CUD pilot uses only 4 models — a full 14-model CUD study would strengthen conclusions
2. Legal domain Content-Order and variance decomposition are pending completion of Paper 6 data collection (currently 2/5 models complete). The information hierarchy is confirmed (Maverick, 50/50 trials) but K(Legal) and Content Fraction cannot yet be determined. Paper 6 will report complete five-domain results independently.
3. Exploration Arc correlations do not reach significance — legal domain data needed
4. Content-Order decomposition uses final position only — position-level analysis may reveal dynamics
5. Philosophy's low ΔRCI makes all decomposition metrics noisy in that domain
6. The three dimensions may not be fully independent — CUD and Content/Order may interact

---

## 5. Conclusion

Context is not a monolith. We decompose LLM context sensitivity into measurable dimensions — depth, structure, stability, and trajectory — using the MCH program's existing three-condition experimental design. The most striking finding is the Content-Order dissociation: content and order both increase sensitivity (ΔRCI) but have **opposite effects on variance** — content amplifies variance 4-6× while order suppresses it to ~30%. The sensitivity decomposition is domain-dependent (medical: 60% content, philosophy: 62% order, d=1.77, p=0.00073), but the variance decomposition is domain-invariant. The conservation product K = ΔRCI × Var_Ratio itself decomposes into content and order components, revealing that the conservation law has internal structure — but domain specificity resides entirely in the sensitivity term, not in the variance machinery. Combined with CUD showing most models are recency-dominant and Exploration Arc showing domain-specific diversity trajectories, these dimensions provide a complete structural account of what "context" means to a large language model. The SCRAMBLED condition — often treated as a mere control — emerges as the key to this decomposition, the shadow dagger hiding in the experimental design from the beginning.

---

## Figures (Planned)

1. **Figure 1**: K-curves for 4 pilot models (medical P30) — rising (Maverick) vs flat (others)
...
|---------|-----------|-------------|
| Content Fraction (ΔRCI): Medical > Philosophy | U=131, p=0.00073, d=1.77 | Highly significant |
| Medical mean Content Fraction | 59.8% ± 9.4% | Content-dominant |
| Philosophy mean Content Fraction | 38.0% ± 14.6% | Order-dominant |
...
| K_total: Medical vs Philosophy | U=46, p=0.003 | Significant |
| K_content: Medical vs Philosophy | U=26, p=0.852 | Not significant (domain-invariant) |
| K_order: Medical vs Philosophy | U=33, p=0.282 | Not significant (domain-invariant) |
| Medical K_total | 0.429 (CV=0.17) | Conservation maintained |
| Philosophy K_total | 0.301 (CV=0.17) | Conservation maintained |
| Llama Scout P30: VR_Content | 16.79 | Extreme content sensitivity |
| Llama Scout P30: VR_Order | 0.44 | Order partially compensates |
| P30 spike in TRUE-COLD | z = +1.88 to +3.30 | Present in all models |
| P30 spike in SCRAMBLED-COLD | z = +0.97 to +3.56 | Present in all models |
| Information hierarchy (legal, Maverick) | 50/50 trials | Preliminary confirmation — Paper 6 in progress (2/5 models) |
| CUD: Maverick depth gradient | K=1: 82% → K=29: 98% | Only rising K-curve |
| Exploration Arc domain contrast | Medical ~1.06, Philosophy ~1.80 (N=6) | Domain-dependent |
