...
% For preprint, use: \usepackage[preprint]{tmlr}
...
% For camera-ready version, fill in actual author info
...
% Camera-ready info (fill in after acceptance)
\def\month{MM}
\def\year{YYYY}
...
Critically, divergent entanglement explains \citeauthor{laban2025lost}'s findings: their +112\% unreliability increase corresponds to models entering divergent regime ($\text{Var\_Ratio} > 1$) as conversation length increases. Models do not ``get lost'' universally---only divergent models exhibit this pattern.
...
All raw data, response text, and analysis scripts are available in the project repository. The experimental protocol, model configurations, and statistical analyses are fully documented. We provide complete position-level data (N=360) and trial-level aggregates to enable independent verification.
...
\newblock {\em Transactions of the Association for Computational Linguistics}, 12, 157--173.
