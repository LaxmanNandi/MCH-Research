...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{fig4_entanglement_multipanel.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.80\textwidth]{fig5_independence_rci_var.png}
...
However, our study comprises 360 controlled observations across 12 model-domain runs. While this scale is sufficient to establish the mechanistic relationship between $\Delta$RCI and variance reduction ($r = 0.76$, $p = 2.37 \times 10^{-68}$), \textbf{operational deployment thresholds require validation at the scale demonstrated by Laban et al.\ (200,000+ conversations)}. The ESI framework provides a mechanistic basis for future large-scale screening protocols, but threshold validation (e.g., ESI $< 1.0$ as instability predictor) must precede clinical deployment recommendations.
...
\begin{itemize}
    \item Var\_Ratio trajectories across conversation lengths
    \item ESI values at task-critical positions
...
\textbf{Scale of evidence.} Our controlled experimental design (360 observations, 50 trials per position) establishes the mechanistic relationship between context sensitivity and variance reduction. However, Laban et al.\ \citeyearpar{laban2025lost} documented the ``Lost in Conversation'' phenomenon across 200,000+ real-world conversations. Our study provides mechanistic explanation for their empirical findings but does not replace their large-scale evidence. Operational deployment recommendations require validation at comparable scale.
...
\begin{itemize}
    \item \textbf{Figure S1}: Gaussian assumption verification for ΔRCI distributions
    \item \textbf{Figure S2}: Trial-level convergence analysis (50-trial adequacy)
    \item \textbf{Figure S3}: Model-level ΔRCI comparison across architectures
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figure6_gaussian_verification.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figure8_trial_convergence.png}
    ...
    Rolling 5-trial means show stable estimates throughout. Slope tests: 11/12 models
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figure9_model_comparison.png}
    \caption{\textbf{Figure S3: Model-level $\Delta$RCI comparison.}
    Mean $\Delta$RCI values by model with 95\% confidence intervals.
    Medical models (closed-goal): mean $\Delta$RCI $= 0.361$;
    Philosophy models (open-goal): mean $\Delta$RCI $= 0.305$.
