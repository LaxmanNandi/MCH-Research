#!/usr/bin/env python3
"""MCH Medical Experiment - Claude Opus 4.5 (50 trials)."""
...
def run_trial(trial_num):
    print(f"  [Claude Opus] Trial {trial_num+1}/{N_TRIALS}...", flush=True)
    ...
    print(f"  [Claude Opus] Trial {trial_num+1} dRCI={delta_rci_cold:.4f}", flush=True)
...
def main():
    print("="*70, flush=True)
    print("MCH MEDICAL REASONING EXPERIMENT - CLAUDE OPUS 4.5", flush=True)
    ...
    if os.path.exists(final_file):
        print(f"  Claude Opus already complete. Skipping.", flush=True)
        return
    ...
    print(f"\n  Claude Opus COMPLETE: {len(valid)}/{N_TRIALS} valid trials", flush=True)
