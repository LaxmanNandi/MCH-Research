#!/usr/bin/env python3
"""MCH Medical Experiment - Sequential execution (one model at a time)."""
...
# Models to run (sequential order)
MODELS_TO_RUN = [
...
def get_response(vendor, model_id, messages):
    if vendor == "openai":
        ...
        return get_response_anthropic(model_id, anthropic_messages)
...
def main():
    print("="*70, flush=True)
    print("MCH MEDICAL REASONING EXPERIMENT - SEQUENTIAL", flush=True)
