#!/usr/bin/env python3
"""
Independence Test: Base Capability (RCI_COLD) vs Entanglement (Var_Ratio)

TRUE TEST of independence:
- Axis 1: RCI_COLD = baseline performance without context
- Axis 2: Var_Ratio = variance(TRUE) / variance(COLD)

These are NOT mathematically linked (unlike dRCI which = 1 - RCI_COLD)
This is the proper test of dual-axis independence.
"""
...
print("="*80)
print("INDEPENDENCE TEST: RCI_COLD vs VAR_RATIO")
print("="*80)
...
# Models with full response data
models = {
    # Philosophy
...
# ============================================================================
# COMPUTE VAR_RATIO PER POSITION
# ============================================================================

all_data = []

print("\nComputing Var_Ratio for each model/position...")
...
for model_name, file_path in models.items():
    ...
    for pos in range(n_prompts):
        # Collect embeddings across all trials for this position
        true_embs = []
        cold_embs = []
        cold_aligns = []
        ...
        for trial in data['trials']:
            # Get responses
            true_resp = trial['responses']['true'][pos]
            cold_resp = trial['responses']['cold'][pos]

            # Embed
            true_embs.append(embedder.encode(true_resp, convert_to_numpy=True))
            cold_embs.append(embedder.encode(cold_resp, convert_to_numpy=True))

            # RCI_COLD
            cold_aligns.append(trial['alignments']['cold'][pos])

        # Convert to arrays
        true_embs = np.array(true_embs)  # shape: (n_trials, 384)
        cold_embs = np.array(cold_embs)

        # Compute variance (mean across embedding dimensions, then across trials)
        var_true = np.var(true_embs, axis=0).mean()
        var_cold = np.var(cold_embs, axis=0).mean()
        ...
        all_data.append({
            ...
            'rci_cold': rci_cold,
            'var_ratio': var_ratio,
            'mutual_info_proxy': 1.0 - var_ratio if not np.isnan(var_ratio) else np.nan
        })
...
print("\n" + "="*80)
print("TEST 1: POOLED CORRELATION (RCI_COLD vs VAR_RATIO)")
print("="*80)

r_pooled, p_pooled = pearsonr(df['rci_cold'], df['var_ratio'])
...
for model_name in df['model'].unique():
    ...
    r, p = pearsonr(model_df['rci_cold'], model_df['var_ratio'])
...
print(f"  Significant: {model_corr_df['significant'].sum()}/{len(model_corr_df)}")
...
for domain in ['Philosophy', 'Medical']:
    ...
    r, p = pearsonr(domain_df['rci_cold'], domain_df['var_ratio'])
    print(f"\n{domain}:")
...
# Model: var_ratio ~ rci_cold + domain + position
...
print("\nRegression: Var_Ratio ~ RCI_COLD + Domain + Position")
print(model.summary())
...
# Feature matrix
features = df[['rci_cold', 'var_ratio']].values
...
print(f"  Var_Ratio:  {pca.components_[0,1]:+.3f}   {pca.components_[1,1]:+.3f}")
...
if loading_corr < 0.3:
    print("  RESULT: Orthogonal (suggests independence)")
else:
    print("  RESULT: Non-orthogonal")
...
for domain in ['Philosophy', 'Medical']:
    domain_df = df[df['domain'] == domain]
    ax.scatter(domain_df['rci_cold'], domain_df['var_ratio'],
              alpha=0.5, s=30, c=colors[domain], label=domain)

# Regression line
x_range = np.linspace(df['rci_cold'].min(), df['rci_cold'].max(), 100)
slope, intercept = np.polyfit(df['rci_cold'], df['var_ratio'], 1)
...
ax.set_ylabel('Variance Ratio (Entanglement)', fontsize=11)
ax.set_title(f'Pooled Correlation (N={len(df)})\nr = {r_pooled:+.3f}, p = {p_pooled:.4e}', fontsize=12)
ax.legend()
ax.grid(alpha=0.3)
ax.axhline(1.0, color='gray', linestyle=':', linewidth=1, label='Var_Ratio=1')
...
for domain in ['Philosophy', 'Medical']:
    domain_df = df[df['domain'] == domain]
    r_domain, p_domain = pearsonr(domain_df['rci_cold'], domain_df['var_ratio'])

    ax.scatter(domain_df['rci_cold'], domain_df['var_ratio'],
              alpha=0.6, s=40, c=colors[domain], label=f'{domain}\nr={r_domain:+.3f}, p={p_domain:.3e}')

    # Fit line
    slope, intercept = np.polyfit(domain_df['rci_cold'], domain_df['var_ratio'], 1)
...
ax.arrow(0, 0, pca.components_[0,1]*scale, pca.components_[1,1]*scale,
        head_width=0.3, head_length=0.3, fc='blue', ec='blue', linewidth=2, label='Var_Ratio')
...
plt.tight_layout()
plt.savefig('c:/Users/barla/mch_experiments/analysis/independence_var_ratio.png', dpi=300, bbox_inches='tight')
print("[OK] Plot saved: independence_var_ratio.png")
...
results_df = pd.DataFrame({
    ...
    'r': [r_pooled] + [pearsonr(df[df['domain']==d]['rci_cold'], df[df['domain']==d]['var_ratio'])[0]
          for d in ['Philosophy', 'Medical']],
    'p': [p_pooled] + [pearsonr(df[df['domain']==d]['rci_cold'], df[df['domain']==d]['var_ratio'])[1]
          for d in ['Philosophy', 'Medical']],
    'N': [len(df), len(df[df['domain']=='Philosophy']), len(df[df['domain']=='Medical'])]
})

results_df.to_csv('c:/Users/barla/mch_experiments/analysis/independence_var_ratio_results.csv', index=False)
print("[OK] Results saved: independence_var_ratio_results.csv")

model_corr_df.to_csv('c:/Users/barla/mch_experiments/analysis/independence_var_ratio_per_model.csv', index=False)
print("[OK] Per-model results saved")

# Save full data
df.to_csv('c:/Users/barla/mch_experiments/analysis/independence_var_ratio_data.csv', index=False)
print("[OK] Full data saved")
...
if independence_supported:
    ...
    conclusion = "Independence SUPPORTED"
    ...
    conclusion = "Independence NOT SUPPORTED"
    ...
    conclusion = "Independence PARTIALLY SUPPORTED"
    reason = f"Weak-to-moderate correlation (|r|={abs(r_pooled):.3f}), marginal significance"

print(f"{'='*80}")

# Save summary
with open('c:/Users/barla/mch_experiments/analysis/independence_var_ratio_summary.md', 'w', encoding='utf-8') as f:
    f.write("# Independence Test: RCI_COLD vs Variance Ratio\n\n")
    f.write("## Summary\n\n")
    f.write(f"**CONCLUSION: {conclusion}** - {reason}\n\n")
    ...
    for domain in ['Philosophy', 'Medical']:
        domain_df = df[df['domain'] == domain]
        r_d, p_d = pearsonr(domain_df['rci_cold'], domain_df['var_ratio'])
    ...
    f.write("This test examines whether RCI_COLD (base capability) and Variance Ratio ")
    f.write("(entanglement measure) are independent dimensions.\n\n")
    f.write("Unlike DRCI (which = 1 - RCI_COLD by construction), Variance Ratio is ")
    f.write("computed independently from embeddings and is NOT mathematically linked to RCI_COLD.\n\n")
    f.write(f"Result: {conclusion}\n")

print("[OK] Summary saved: independence_var_ratio_summary.md")
