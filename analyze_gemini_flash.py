"""
MCH v8.1 Comprehensive Analysis - Gemini 2.5 Flash (100 Trials)
...
def analyze_gemini_flash():
    print("=" * 70)
    print("MCH v8.1 COMPREHENSIVE ANALYSIS - GEMINI 2.5 FLASH")
    print("=" * 70)

    # Load results
    results = load_results("mch_results_gemini_flash_100trials.json")
    ...
    print("\n" + "=" * 70)
    print("PRIMARY HYPOTHESIS TEST: ΔRCI vs Cold")
    print("=" * 70)
    ...
    print(f"\nMean ΔRCI (True - Cold): {mean_drci:.4f}")
    ...
    if p_val < 0.05:
        if mean_drci > 0:
            classification = "CONVERGENT (Positive ΔRCI, p < 0.05)"
            pattern = "Convergent"
        else:
            classification = "SOVEREIGN (Negative ΔRCI, p < 0.05)"
            pattern = "Sovereign"
    else:
    ...
    # Scrambled condition
    print("\n" + "-" * 50)
    print("SECONDARY: ΔRCI vs Scrambled")
    print("-" * 50)
    ...
    print(f"Mean ΔRCI (True - Scrambled): {mean_drci_s:.4f}")
    ...
    print(f"\nFirst 50 Trials:")
    print(f"  Mean ΔRCI: {mean_f50:.4f} (SD: {std_f50:.4f})")

    print(f"\nLast 50 Trials:")
    print(f"  Mean ΔRCI: {mean_l50:.4f} (SD: {std_l50:.4f})")
    ...
    fig.suptitle(f'MCH v8.1 Analysis: Gemini 2.5 Flash (n={n})\nPattern: {pattern} | Cohen\'s d = {cohens_d:.3f} | p = {p_val:.4f}',
                 fontsize=14, fontweight='bold')

    trial_nums = list(range(1, n + 1))

    # Panel 1: ΔRCI over trials (cold)
    ax1 = axes[0, 0]
    ...
    ax1.set_title('ΔRCI Over Trials (vs Cold)')
    ax1.legend(fontsize=8)
    ax1.grid(True, alpha=0.3)

    # Panel 2: ΔRCI histogram
    ax2 = axes[0, 1]
    ...
    ax2.set_title('ΔRCI Distribution')
    ...
    # Panel 8: Rolling mean ΔRCI (window=10)
    ax8 = axes[2, 1]
    window = 10
    ...
    ax8.set_ylabel('Rolling Mean ΔRCI')
    ax8.set_title(f'Rolling Mean ΔRCI (window={window})')
    ...
    Key Finding:
      Gemini Flash shows NEGATIVE
      context utilization - performs
      WORSE with conversation history.
    """
    ...
    # Save figure
    output_file = 'gemini_flash_mch_analysis.png'
    ...
    print(f"""
    GEMINI 2.5 FLASH - MCH v8.1 EXPERIMENT RESULTS
    ═══════════════════════════════════════════════

    Pattern Classification: {classification}

    Key Finding: Gemini Flash exhibits a SOVEREIGN pattern,
    meaning it performs WORSE when given conversation history
    compared to "cold" (no history) responses.

    Statistical Evidence:
    - ΔRCI = {mean_drci:.4f} (negative = worse with context)
    - p = {p_val:.6f} (highly significant, p < 0.01)
    - Cohen's d = {cohens_d:.3f} ({effect_interp} effect)
    - 95% CI does NOT include zero: [{ci_95[0]:.4f}, {ci_95[1]:.4f}]

    Interpretation:
    Unlike Claude Opus 4 (Convergent, +ΔRCI) which benefits from
    context history, Gemini Flash's responses become LESS aligned
    with prompts when provided with conversation context.

    This suggests fundamentally different context-processing
    architectures between these models.
    """)
