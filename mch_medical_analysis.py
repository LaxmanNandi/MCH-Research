"""
MCH Medical Reasoning Domain Analysis
Complete Statistical Analysis for Medical Clinical Reasoning
...
DATA_DIR = "C:/Users/barla/mch_experiments/medical_results"
...
OUTPUT_DIR = "C:/Users/barla/mch_experiments/medical_results/analysis"
...
def load_all_data():
    ...
    Note: Models with 0 valid trials are excluded (e.g., Gemini Pro blocked by safety filters).
    """
    data = {}
    ...
    for model_name, filename in DATA_FILES.items():
        ...
        try:
            ...
            # Filter invalid trials (dRCI=0 indicates API failure/blocked)
            valid_trials = [t for t in trials if abs(t['delta_rci']['cold']) > 0.0001]
            
            # Skip models with no valid data
            if len(valid_trials) == 0:
                print(f"  SKIPPED {model_name}: 0/{len(trials)} valid (safety filter blocked)")
                continue

            # Extract metrics from VALID trials only
            delta_rci_cold = [t['delta_rci']['cold'] for t in valid_trials]
            delta_rci_scrambled = [t['delta_rci']['scrambled'] for t in valid_trials]
            true_alignments = [t['alignments']['mean_true'] for t in valid_trials]
            cold_alignments = [t['alignments']['mean_cold'] for t in valid_trials]
            scrambled_alignments = [t['alignments']['mean_scrambled'] for t in valid_trials]
            ...
            print(f"  Loaded {model_name}: {len(valid_trials)}/{len(trials)} valid trials")
...
def create_figure1_violin(data, output_dir):
    ...
    ax.set_title('MCH Medical Domain: Response Coherence by Model\n(Positive = Convergent, Negative = Sovereign)', fontsize=14)
...
def create_figure2_forest(data, output_dir):
    ...
    ax.set_title('MCH Medical Domain: Effect Sizes with 95% Confidence Intervals\n(CIs crossing zero = Neutral pattern)', fontsize=14)
...
def create_figure3_vendor_tier(data, output_dir):
    ...
    fig.suptitle('MCH Medical Domain: Vendor and Tier Effects', fontsize=14, fontweight='bold')
...
def generate_report(data, normality, wilcoxon, anova, scrambled, correlations, output_dir):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("MCH MEDICAL REASONING DOMAIN ANALYSIS REPORT\n")
        f.write("Medical Clinical Reasoning Experiment (STEMI Case)\n")
        f.write("=" * 80 + "\n\n")

        f.write(f"Generated: 2026-01-12\n")
        f.write(f"Models Analyzed: {len(data)}\n")
        f.write(f"Trials per Model: 50\n\n")
...
def main():
    print("=" * 70)
    print("MCH MEDICAL REASONING DOMAIN ANALYSIS")
    print("Complete Statistical Analysis for Medical Clinical Reasoning")
    print("=" * 70)
    ...
    if len(data) < 4:
        print(f"ERROR: Only loaded {len(data)} models. Need at least 4 for analysis.")
        return
    
    print(f"
Note: {len(data)} models loaded for analysis.")
    if len(data) < 6:
        print("  (Some models excluded due to safety filters or API issues).")
    ...
    print("\n" + "=" * 70)
    print("MEDICAL DOMAIN ANALYSIS: DONE")
    print("Ready for cross-domain comparison with Philosophy!")
    print("=" * 70)
